...
function updateStyles(extension) {
    const styleElement = extension.storage.styleElement;
    if (!styleElement)
        return;
...
function setupObservers(extension) {
...
function refreshPage(extension, targetNode) {
...
function createPageBreak({ firstPage, pageIndex, pageOptions, }) {
...
function createDecoration(state, pageOptions) {
    const pageWidget = Decoration.widget(0, (view) => {
        ...
        const actualPageContentHeight = totalHeight - previousPageCount * (totalPageGap + pageOptions.pageGapBorderSize * 2);
...
export const PaginationPlus = Extension.create({
    ...
    onDestroy() {
        ...
        if ((_a = this.storage.styleElement) === null || _a === void 0 ? void 0 : _a.parentNode) {
