...
// Ensure window.footnoteUpdateTimer is declared with the correct type
declare global {
  interface Window {
...
export const PaginationPlus = Extension.create<PaginationPlusOptions>({
  ...
  addOptions() {
    return {
      ...
      pageHeaderHeight: 10,
      pageFooterHeight: 70, // Increased to accommodate footnotes
      pageMarginLeft: 0,
      pageMarginRight: 0,
      footerText: "",
  ...
  onCreate() {
    ...
    const refreshPage = (targetNode: HTMLElement) => {
      ...
      // Trigger footnote update after page refresh
      if (window.footnoteUpdateTimer) {
        clearTimeout(window.footnoteUpdateTimer)
      }

      // Use a shorter initial delay for the first update
      setTimeout(() => {
        const event = new CustomEvent("paginationUpdated")
        document.dispatchEvent(event)
      }, 100)

      // Then do another update after layout has settled
  ...
  const pageWidget = Decoration.widget(
    ...
    (view) => {
      ...
      const pageBreakDefinition = ({
        ...
        // Add footnote container if enabled
        if (pageOptions.enableFootnotes) {
          ...
          footnoteContainer.style.bottom = "5px"
          footnoteContainer.style.left = "25px"
          footnoteContainer.style.right = "25px"
          footnoteContainer.style.borderTop = "1px solid #ddd"
          footnoteContainer.style.paddingTop = "5px"
          footnoteContainer.style.fontSize = "0.85em"
          footnoteContainer.style.display = "none" // Initially hidden
          footnoteContainer.style.zIndex = "100"
          pageFooter.appendChild(footnoteContainer)
        }
